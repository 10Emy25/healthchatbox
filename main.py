...
【语言自适应】
请严格根据用户输入的语言（中文或英文）来决定你所有输出使用的语言。
- 如果用户输入是中文，你的所有追问、最终 JSON 中的 "advice" 字段都用中文。
- 如果用户输入是英文，你的所有追问、最终 JSON 中的 "advice" 字段都用英文。
- 其他字段（如"主诉"、"标准化症状"等）按原样保留中文，但 "advice" 必须跟随用户语言。
